# SlmFlowFramework: фреймворк для динамической маршрутизации SLM/LLM в RAG-пайплайнах

## Введение

Современные производственные системы на основе Retrieval-Augmented Generation (RAG) сталкиваются с фундаментальным противоречием: большие языковые модели (LLM) обеспечивают высокое качество ответов, но дороги в эксплуатации, тогда как малые языковые модели (SLM) значительно дешевле, однако справляются не со всеми задачами. Наивное решение — запускать LLM для всех запросов — неоправданно дорого; отправлять всё в SLM — значит принимать потери качества на сложных примерах.

**SlmFlowFramework** решает задачу интеллектуальной маршрутизации: для каждого входящего запроса в режиме реального времени определяется, достаточно ли SLM или необходимо привлекать LLM. Фреймворк предоставляет полный инструментарий для постановки эксперимента: генерацию синтетических размеченных данных, реализацию нескольких стратегий маршрутизации и многоуровневую оценку качества.

### Задачи RAG, охватываемые фреймворком

- **Reranking** — ранжирование набора документов по релевантности к запросу; правильный ответ — индекс наиболее релевантного документа.
- **Context Compression** — сжатие набора документов в компактный контекст, сохраняющий всю информацию, необходимую для ответа на запрос.

Обе задачи существенно различаются по природе трудности: reranking требует понимания смысловой близости, compression — умения выделять и синтезировать ключевые факты.
...
| `core/utils/` | Постобработка метрик, экспорт результатов |
...
Датасет организован по трём измерениям, образующим декартово произведение комбинаций:

| Измерение | Значения |
|-----------|---------|
| Задача (`task`) | `reranking`, `context_compression` |
...
`RAGDatasetAsyncGenerator` реализует конкурентную генерацию с автоматическим retry:
...
Языковые модели склонны отвечать в произвольном формате. В production-системе это недопустимо: парсинг произвольного текста ненадёжен, а структурированные данные необходимы на каждом шаге — при генерации датасета, инференсе и оценке.

### Решение: LangchainMessageBuilder + PydanticOutputParser

Все LLM-вызовы в фреймворке используют единый паттерн структурированного вывода:
...
`{fmt}` в шаблоне заменяется на инструкции формата, сгенерированные `PydanticOutputParser.get_format_instructions()`. Это стандартизирует ожидаемый формат ответа непосредственно внутри промпта.
...
Для принятия решения о маршрутизации необходимо численно охарактеризовать сложность задачи. `RAGFeatureExtractor` вычисляет два типа векторов признаков — по одному для каждой задачи.
...
- **wordfreq** — средняя частотность слов в запросе (мера редкости/специализированности лексики)

### Общие признаки (оба вектора)

| Признак | Описание | Интерпретация |
|---------|---------|---------------|
| `query_token_count` | Количество токенов в запросе | Чем длиннее запрос, тем сложнее задача |
...
| `avg_lexical_overlap` | Средний лексический overlap запрос/документы | Низкий overlap → сложнее найти релевантный документ |
...
| `documents_cosine_similarity` | Среднее попарное косинусное сходство документов | Высокое сходство → документы похожи, реранкинг труден |
...
| `relevant_documents_ratio` | Доля документов с overlap ≥ 0.3 | Мало релевантных документов → сложнее найти нужное |
...
Маршрутизатор реализует единый интерфейс протокола `Routable`:
...
Решение принимается по двойному критерию — оба условия должны выполняться одновременно:

1. Сработало не менее `min_triggers` правил
2. Сумма весов сработавших правил ≥ `cumulative_weights_threshold`

Двойной критерий защищает от ложных срабатываний: одиночный сильный сигнал не может переопределить итоговое решение без подтверждения со стороны других признаков.
...
**Правила для context_compression** дополнительно учитывают `total_context_token_count`, `avg_chunk_token_count` и `relevant_documents_ratio`.

**Преимущества:** интерпретируемость, нулевая латентность (нет дополнительного LLM-вызова), легко настраивать пороги.
...
При `OutputParserException` политика консервативно возвращает `confidence=0` (SLM справится).

**Преимущества:** не требует ручного конструирования признаков; адаптируется к семантической сложности.

**Ограничения:** добавляет один LLM-вызов к каждому запросу (latency overhead); качество зависит от способности роутера-SLM корректно калибровать сложность.
...
В режиме `"dynamic"` роутер автоматически определяет тип политики: `WeightedRuleBasedRoutingPolicy` принимает на вход **признаковый вектор**, `SLMRoutingPolicy` — **сырой `RAGTask`** (для рендеринга промпта).
...
Для каждого `InferenceRecord` параллельно вычисляются:

**Автоматические метрики:**

- **BERTScore F1** — семантическое сходство предсказания с golden_answer через BERT-эмбеддинги
- **ROUGE-L** (только для compression) — longest common subsequence overlap
- **ROUGE-2** (только для compression) — биграмный overlap
- **Compression Ratio** — `tokens(prediction) / total_context_tokens`; для compression: насколько эффективно модель сжала контекст
...
`final_score = min(...)` — консервативная агрегация: один слабый критерий тянет итоговую оценку вниз. Это препятствует ситуации, когда модель получает высокий балл за полноту, скрывая галлюцинации.

Судья получает контекст о задаче из `TASK_DESCRIPTIONS`, что позволяет оценивать reranking и compression по релевантным для каждой задачи критериям.
...
Эти два числа отражают ключевой trade-off: `slm_routing_ratio` — экономия ресурсов, `slm_success_ratio` — качество при экономии.
...
Такое поведение предотвращает распространение ошибок при сохранении работоспособности всего пайплайна.

### Загрузка датасета

`RAGSyntheticDataset.from_files()` рекурсивно обходит директорию `slm_flow_df/`, восстанавливая метаданные из структуры пути (task, domain, difficulty). После загрузки датасет автоматически перемешивается для безопасного использования в экспериментах.

---

## Расширяемость

Фреймворк спроектирован для минимального boilerplate при добавлении новых компонентов:

| Что добавить | Что реализовать |
|-------------|----------------|
| Новая политика маршрутизации | Реализовать протокол `Routable` с методом `call_large_model()` |
| Новая задача RAG | Добавить промпт в `PROMPT_REGISTRY`, признаки в `RAGFeatureExtractor`, метрики в `RAGPipelineRunner` |
| Новые метрики оценки | Расширить `_compute_reranking_metrics()` или `_compute_compression_metrics()` |
...
1. **Генерация синтетических данных** — покрывает пространство task × domain × difficulty с автоматическим retry и типизированными выходами
2. **Две комплементарные стратегии маршрутизации** — rule-based (интерпретируемость, нулевая латентность) и SLM-as-router (адаптивность, семантическое понимание сложности)
3. **Многоуровневая оценка** — автоматические метрики (BERTScore, ROUGE) + LLM-судья с консервативной агрегацией
4. **Модульная архитектура** — каждый компонент заменяем независимо; добавление новой политики или задачи требует минимальных изменений кода
