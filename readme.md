...
**`data/synthetic.py`** — генерирует RAG-примеры через OpenAI с ограничением параллелизма (asyncio Semaphore), сохраняет JSON-чекпоинты по пути `slm_flow_df/{task}/{domain}/{difficulty}/{uuid}.json`.

**`scheduler/`** — центральный модуль маршрутизации. В режиме `dynamic` извлекает числовые признаки из запроса и документов, затем применяет политику (`ThresholdRoutingPolicy` или `MLRoutingPolicy`) для выбора SLM или LLM.
